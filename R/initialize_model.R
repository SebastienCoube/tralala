...
plotTbf = function(tbf, log.x = T){
  plot(1, 1, type = "n", ylim = c(0,1), xlim = c(1, max(max(tbf)*1.1, max(tbf) + 2)), log = c("", "x")[1+log.x],
  ...
  abline(v=max(tbf), col = 1)
  text(x = max(tbf)*1.05, y = .5, labels = "constant after this time counter", srt = 90)
  ...
  if(length(tbf)>2){
    for(i in seq(2, length(tbf)-1)){
      points(seq(tbf[i-1], tbf[i]), seq(0, 1, length.out = tbf[i]- tbf[i-1] +1), type = "b", col = i)
      points(seq(tbf[i], tbf[i+1]), seq(1,0, length.out = tbf[i+1]- tbf[i] +1), type = "b", col = i)
    }
  }
}
...
beginTransitionModel = function(
  ...
  -Each row of the table must have at least one non-NA slot except for absorbing states"
  )
  message("----------")
...
beginEmissionModel = function(
  ...
  if(is.null(emission_log_likelihood_grad)){
    message("The argument emission_log_likelihood_grad was not provided and is obtained by applying finite differences on emission_log_likelihood")
    emission_log_likelihood_grad = function(estimated_emission_param_vec, fixed_emission_param_vec, emission, explanatory_variable_vec){
