...
preprocessTbf = function(transition_model, params){
  ...
  semi_markov_depths_per_state = sapply(
    transition_model$dont_touch$latent_states_names,
    function(state){
      if(is_state_absorbing[state])return(1)
      return(max(
        sapply(
          tbf_used_per_state[[state]],
          function(tbf_name){
            max(transition_model$dont_touch$temporal_basis_function_list[[(tbf_name)]])
          }
        )
      ))
    }
...
sampleLatentState = function(n_samples, initial_latent_state = NULL, initial_time_counter = NULL,  model, params,  data_seq){
  if((!is.null(initial_latent_state)) | (!is.null(initial_time_counter))){
    if(length(initial_latent_state)!= length(initial_time_counter))stop("initial_latent_state and initial_time_counter must have the same length")
    if(any(is.na(match(initial_latent_state, model$transition_model$dont_touch$latent_states_names))))stop(paste("initial_latent_state must be a vector with elements", model$transition_model$dont_touch$latent_states_names))
    ...
    n_samples = length(initial_latent_state)
  }

  preprocessed_tbf = preprocessTbf(transition_model = model$transition_model, params = params)
  n_time_periods = nrow(data_seq$explanatory_variables_transition) + 1
  # drawing initial states
  latent_state =
    matrix("0", n_time_periods, n_samples)
  if(is.null(initial_latent_state)) initial_latent_state = model$transition_model$dont_touch$latent_states_names[cut(runif(n_samples), c(0, seq(model$transition_model$dont_touch$n_latent_states)/model$transition_model$dont_touch$n_latent_states))]
  latent_state[1,] = initial_latent_state
  time_counter =
    ...
    for(s in model$transition_model$dont_touch$latent_states_names[which(!preprocessed_tbf$is_state_absorbing)] ){
      ...
      for(s_ in preprocessed_tbf$exit_states[[s]]){
        preprocessed_tbf$softmax_matrices[[s]][,s_] =
          exp(preprocessed_tbf$pre_multiplied_tbfs[[s]][[s_]] %*% data_seq$explanatory_variables_transition[time_idx-1, preprocessed_tbf$active_explanatory_variables[[s]]])
      ...
      # print(preprocessed_tbf$softmax_matrices[[s]] %*% c(rep(1, ncol(preprocessed_tbf$softmax_matrices[[s]])-1), 0) + 1/preprocessed_tbf$softmax_matrices[[s]][,ncol(preprocessed_tbf$softmax_matrices[[s]])])
    }
    for(s in model$transition_model$dont_touch$latent_states_names[which(preprocessed_tbf$is_state_absorbing)] ){
...
sampleEmissions = function(data_seq, model, params, latent_state_seq, emission_sampler){
  ...
  if(any(is.na(match(formalArgs(emission_sampler), argnames))))stop(paste(
    "The allowed formal arguments of the function emission_sampler are :",
...
addEmissionsInDataList = function(
    data_list,
    model,
    params,
    emission_sampler,
    initial_latent_state = NULL,
    initial_time_counter = NULL
    ){
  for(n in seq(length(data_list))){
    latent_state_seq = sampleLatentState(
      n_samples = 1, initial_latent_state = initial_latent_state[n],
      initial_time_counter = initial_time_counter[n],
      model = model, params = params, data_seq = data_list[[n]])
    data_list[[n]]$latent_states = latent_state_seq
    data_list[[n]]$emissions =
      sampleEmissions(
        data_seq = data_list[[n]],
        model = model, params = params,
        latent_state_seq = latent_state_seq$latent_state,
